...
public class HashMap<K,V> implements MutableMap<K,V>{
    ...
    public Iterator<MutableEntry<K, V>> iterator() {
        ...
        return new Iterator<MutableEntry<K, V>>() {
            int currentPos= 0;
            Node<K,V> nodeIt= null;
            public boolean hasNext() {
                if (nodeIt != null) return true;
                for(; currentPos < table.length; currentPos++) {
                   if(table[currentPos] != null) { nodeIt = table[currentPos]; return true; }
                }
                return false;
            }
            public MutableMap.MutableEntry<K, V> next() {
                hasNext(); Node<K,V> aux = nodeIt; nodeIt = null; return aux;
            }
        };
    }
